...
static void spi_kona_work(struct work_struct *work)
{
...
#ifndef CONFIG_ISDBT //2013.04.09 hs.yi@latin block spi multi transfer concept for isdbt spi communication.
	while (!list_empty(&spi_kona->queue)) {
		...
		if (spi_kona->do_setup == 1) {
			...
			if (status < 0)
#ifdef CONFIG_ISDBT //2013.04.09 hs.yi@latin block spi multi transfer concept for isdbt spi communication.
			goto FINISH;
#else
				break;
#endif
