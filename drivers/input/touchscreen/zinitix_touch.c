...
#include "./ZinitixFW/zinitix_touch_zxt_firmware(ZI02FF80).h"
#include "./ZinitixFW/zinitix_touch_zxt_firmware(ZI03221F).h"
...
const u8 *m_pFirmware [TSP_TYPE_COUNT] = {(u8*)m_firmware_data_03,(u8*)m_firmware_data_02,};
u8 m_FirmwareIdx = 0;
...
static struct device *sec_touchscreen_dev;
static struct device *sec_touchkey_dev;
...
static void get_x_num(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%u", info->cap_info.x_node_num - 1);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
