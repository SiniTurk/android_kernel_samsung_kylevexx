...
void musb_vbus_changed_tsu6111(int state)
{
	if( !local_usbsw ){
...
EXPORT_SYMBOL(musb_vbus_changed_tsu6111);


unsigned int musb_get_charger_type_tsu6111(void)
{
	if( !local_usbsw ){
		printk("failed to allocate driver data\n");
...
EXPORT_SYMBOL(musb_get_charger_type_tsu6111);
...
int bcm_ext_bc_status_tsu6111(void)
{
	return tsu6111_get_chrgr_type();
}
EXPORT_SYMBOL(bcm_ext_bc_status_tsu6111);
...
#if defined(CONFIG_MACH_HAWAII_SS_LOGAN_REV03)
