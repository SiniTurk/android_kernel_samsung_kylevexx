...
#ifdef H264_CLK_MGMT
#ifdef H264_QOS_MGMT
		pi_mgr_qos_request_update(&h264_qos_node,
			PI_MGR_QOS_DEFAULT_VALUE);
		scu_standby(1);
#endif

		disable_h264_clock();
		if (pi_mgr_dfs_request_update(&h264_dfs_node,
				PI_MGR_DFS_MIN_VALUE)) {
			printk(KERN_ERR "%s: failed to update dfs request for h264\n",
				__func__);
		}
...
#ifdef H264_CLK_MGMT
		ret =
			pi_mgr_dfs_add_request(&h264_dfs_node, "h264",
...
#endif
		usleep_range(50, 100); /* buffer to ensure above are done. */
